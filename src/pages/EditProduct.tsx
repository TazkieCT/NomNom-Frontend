...
interface Food {
  ...
  isAvailable: boolean
  images: string[]
  filters: string[] | { _id: string; name: string }[]
}
...
export default function EditProduct() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const [categoriesRes, filtersRes, foodRes] = await Promise.all([
        ...
        if (categoriesRes.ok && filtersRes.ok && foodRes.ok) {
          ...
          setFormData({
            ...
            categoryId: typeof foodData.categoryId === 'string' ? foodData.categoryId : foodData.categoryId._id,
            isAvailable: foodData.isAvailable,
            images: foodData.images.length > 0 ? foodData.images : [""],
            filters: Array.isArray(foodData.filters) 
              ? foodData.filters.map(f => typeof f === 'string' ? f : f._id)
              : []
          })
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        navigate('/manage-products', { state: { message: 'Product updated successfully!' } })
      } else {
